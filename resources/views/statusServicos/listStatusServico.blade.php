                        ...
                        @foreach ($lista as $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    {{-- <form action="{{ route('statusServicos.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header bg-danger">
                    <h5 class="modal-title text-white font-weight-bold" id="deleteModalTitle">{{ __('Excluir Status Serviço') }}</h5>
                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="/statusServicos" method="POST" id="deleteForm">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            table.on('click', '.delete', function() {
                ...
                $('#deleteForm').attr('action', '/statusServicos/' + data[0]);
